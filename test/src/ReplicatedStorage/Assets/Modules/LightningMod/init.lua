...
local LightningBolt = class() do
	...
	function LightningBolt:init(origin, goal, options)
		...
		self.tweendecayinfo = self.options.tweendecayinfo or nil
	...
	function LightningBolt:Draw(parent)
		...
		for i = 1, #lines do
			...
			if self.tweendecayinfo then
				game:GetService"TweenService":Create(part,self.tweendecayinfo.info,self.tweendecayinfo.outcome):Play()
			end
		end
