...
------------------------------------------------------------
	...
	pluginToolbar = plugin:CreateToolbar("Moon Animator 2") 																																																																																												if plugin.Name ~= "cloud_4725618216" then pluginToolbar:CreateButton("", "invalid plugin", "http://www.roblox.com/asset/?id=0") return end
...
do
						...
						if Pose.ClassName == "Pose" and Pose.Weight > 0 and Pose:FindFirstChild("xSIXxNull") == nil and Pose:FindFirstChild("Null") == nil and Pose.Parent ~= Keyframe then
							...
							if joint then
								...
								elseif Pose.EasingStyle.Name ~= "Linear" then
									local ease_type = Pose.EasingStyle.Name
									if ease_type == "CubicV2" then
										ease_type = "Cubic"
									end
									final[id].Easing[frm_pos] = {_tblType = "Ease", ease_type = ease_type, params = {Direction = Pose.EasingDirection.Name}}
								else
						...
						elseif Pose.ClassName == "NumberPose" and Pose.Weight > 0 then
							...
							if face_controls then
								...
								elseif Pose.EasingStyle.Name ~= "Linear" then
									...
									target_tbl[id].Easing[frm_pos] = {_tblType = "Ease", ease_type = ease_type, params = {Direction = Pose.EasingDirection.Name}}
								else
	...
	_g.ExportItemObject = function(ItemObject)
		...
		if ItemObject.RigContainer then
			...
			local function GetPose(roblox_kf, path_tbl)
				...
				for pos, value, ease in KeyframeTrack:TargetValueIterator() do
					if last_pos then
						for p = last_pos + 1, pos - 1, 1 do
							...
							local r_pose = GetPose(r_kf, path_tbl)
							r_pose.Weight = 1
							if is_number then
